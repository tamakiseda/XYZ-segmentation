...
with tbls[1]:
    ...
    try:
        ...
        st.write(f"Elapsed time of the Algorithm: {elapsed_time:.7f} seconds")
        ...
        # Create a pie plot with custom styling
        pie_fig = px.pie(values=df['category'].value_counts(),
                        names=df['category'].value_counts().index,
                        color_discrete_sequence=colors,
                        hole=0.3,  # Adjust the size of the center hole
                        labels={'label': 'Category', 'value': 'Count'},  # Customize labels
                        title='Proportions of Categories')  # Add a title

        # Update layout for better aesthetics
        pie_fig.update_traces(textposition='inside', textinfo='percent+label', pull=[0.05]*len(df['category'].value_counts()))
